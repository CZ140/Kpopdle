...
export default function ResultModal({ gameState, revealedSong, guesses, gameNumber, hintsUsed = 0, isArchive = false, isPractice = false, communityStats = null, gameName, onPlayAgain, onClose }) {
      ...
      >
        ...
        {isArchive && (
          <div className="text-center mb-3">
            <span className="inline-block text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-1 rounded-full" style={{ color: 'color-mix(in srgb, var(--color-primary) 70%, transparent)', background: 'color-mix(in srgb, var(--color-primary) 10%, transparent)' }}>
              Archive #{gameNumber}
            </span>
          </div>
        )}
          ...
          <div className="glass-card rounded-xl p-5 mb-6 text-center">
            ...
            {revealedSong.spotifyId && (
              <a
                ...
                className="inline-flex items-center gap-1.5 mt-4 text-xs font-semibold hover:opacity-75 transition-opacity"
